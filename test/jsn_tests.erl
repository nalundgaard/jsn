...
path_elements_map_test_() ->
    [?_assertEqual(#{},jsn:path_elements_map([])),
     ?_assertEqual(#{foo => true,
                     <<"foo">> => true},
                   jsn:path_elements_map([foo])),
     ?_assertEqual(#{foo => #{bar => true,
                              <<"bar">> => true,
                              first => #{last => true}},
                     <<"foo">> => #{bar => true,
                                    <<"bar">> => true,
                                    first => #{last => true}}},
                   jsn:path_elements_map([<<"foo.bar">>,
                                          {<<"foo">>, first, last}])),
     ?_assertEqual(#{<<"xyz123">> => #{bar => true,
                                       <<"bar">> => true}},
                   jsn:path_elements_map([<<"xyz123.bar">>])),
     ?_assertEqual(#{<<"xyz123">> => #{<<"abc456">> => #{<<"def789">> => true}}},
                   jsn:path_elements_map([<<"xyz123.abc456.def789">>,
                                          <<"xyz123.abc456.def789.ghi012">>])),
     ?_assertError(badarg,
                   jsn:path_elements_map(#{}))].
...
with_test_() ->
    Object = #{<<"foo">> => #{bar => 99,
                              ...
                              <<"green">> => [#{eggs => 2, <<"idx">> => 1},
                                              #{<<"ham">> => 30, idx => 2}]}},
    ProplistObject = jsn:from_map(Object, [{format, proplist}]),
    Eep18Object = jsn:from_map(Object, [{format, eep18}]),
    StructObject = jsn:from_map(Object, [{format, struct}]),
    [?_assertEqual(#{},
                   jsn:with([<<"nothing">>], Object)),
     ?_assertEqual(#{},
                   jsn:with([<<"foo.bar.no_key">>], Object)),
     ?_assertEqual(Object,
                   jsn:with([<<"foo">>, <<"nothing">>], Object)),
     ?_assertEqual(#{<<"foo">> => #{bar => 99,
                                    <<"baz">> => #{<<"fum">> => <<"a">>}}},
                   jsn:with([<<"foo.bar">>,
                             <<"foo.baz.fum">>],
                            Object)),
     ?_assertEqual(#{<<"foo">> => #{<<"green">> => [#{eggs => 2, <<"idx">> => 1}]}},
                   jsn:with([{<<"foo">>, <<"green">>, 1}],
                            Object)),
     ?_assertEqual(#{<<"foo">> => #{bar => 99,
                                    <<"baz">> => #{<<"fum">> => <<"a">>},
                                    <<"green">> => [#{eggs => 2},
                   ...
                   jsn:with([<<"foo.bar">>,
                             <<"foo.baz.fum">>,
                             {<<"foo">>, <<"green">>, first, <<"eggs">>},
                             {<<"foo">>, <<"green">>, last},
                             {<<"foo">>, <<"green">>, 99, <<"ham">>}],
                            Object)),
     ?_assertEqual([],
                   jsn:with([<<"nothing">>], ProplistObject)),
     ?_assertEqual([],
                   jsn:with([<<"foo.bar.nokey">>], ProplistObject)),
     ?_assertEqual(ProplistObject,
                   jsn:with([<<"foo">>, <<"nothing">>], ProplistObject)),
     ?_assertEqual([{<<"foo">>, [{bar, 99},
                                 {<<"baz">>, [{<<"fum">>, <<"a">>}]}]}],
                   jsn:with([<<"foo.bar">>,
                             <<"foo.baz.fum">>],
                            ProplistObject)),
     ?_assertEqual([{<<"foo">>, [{<<"green">>, [[{<<"idx">>, 1},
                                                 {eggs, 2}]]}]}],
                   jsn:with([{<<"foo">>, <<"green">>, 1}],
                            ProplistObject)),
     ?_assertEqual([{<<"foo">>, [{bar, 99},
                   ...
                   jsn:with([<<"foo.bar">>,
                            ...
                            ProplistObject)),
     ?_assertEqual({[]},
                   jsn:with([<<"nothing">>], Eep18Object)),
     ?_assertEqual({[]},
                   jsn:with([<<"foo.bar.nokey">>], Eep18Object)),
     ?_assertEqual(Eep18Object,
                   jsn:with([<<"foo">>, <<"nothing">>], Eep18Object)),
     ?_assertEqual({[{<<"foo">>, {[{bar, 99},
                                   {<<"baz">>, {[{<<"fum">>, <<"a">>}]}}]}}]},
                   jsn:with([<<"foo.bar">>,
                             <<"foo.baz.fum">>],
     ...
     ?_assertEqual({[{<<"foo">>, {[{bar, 99},
                   ...
                   jsn:with([<<"foo.bar">>,
                            ...
                            Eep18Object)),
     ?_assertEqual({struct, []},
                   jsn:with([<<"nothing">>], StructObject)),
     ?_assertEqual({struct,[]},
                   jsn:with([<<"foo.bar.nokey">>], StructObject)),
     ?_assertEqual(StructObject,
                   jsn:with([<<"foo">>, <<"nothing">>], StructObject)),
     ?_assertEqual({struct, [{<<"foo">>,
                              {struct, [{bar, 99},
                                        {<<"baz">>,
                                         {struct, [{<<"fum">>, <<"a">>}]}}]}}]},
                   jsn:with([<<"foo.bar">>,
     ...
     ?_assertEqual(#{},
                   jsn:with([{<<"array">>, 2}],
                             #{array => [1]})),
     ?_assertEqual(#{},
                   jsn:with([{<<"array">>, first, <<"foo">>}],
                            #{array => []})),
     ?_assertEqual(#{},
                   jsn:with([{<<"element">>, last}],
                            #{<<"element">> => true})),
     ?_assertEqual(#{},
                   jsn:with([{<<"array">>, first, <<"foo">>}],
                            #{array => [1]})),
     ?_assertEqual(#{array => [2]},
                   jsn:with([{<<"array">>, last}],
                            #{array => [1,2]})),
     ?_assertEqual(#{array => [#{foo => <<"bar">>}]},
                   jsn:with([{<<"array">>, first}],
                            #{array => [#{foo => <<"bar">>},
                                        #{foo => <<"baz">>}]})),
     ?_assertEqual(#{array => [#{foo => <<"bar">>}]},
                   jsn:with([{<<"array">>, first, <<"foo">>}],
                            #{array => [#{foo => <<"bar">>},
                                        #{foo => <<"baz">>}]})),
     ?_assertError(badarg, jsn:with([], {99})),
     ?_assertError(badarg, jsn:with({99}, #{}))].
...
without_test_() ->
    ...
    StructObject = jsn:from_map(Object, [{format, struct}]),
    [?_assertEqual(Object,
                   jsn:without([<<"nothing">>], Object)),
     ?_assertEqual(#{},
                   jsn:without([<<"foo">>, <<"nothing">>], Object)),
     ?_assertEqual(#{<<"foo">> => #{<<"baz">> => #{<<"fum">> => <<"a">>,
                                                   <<"fun">> => <<"b">>}}},
                   jsn:without([<<"foo.bar">>, [foo, green]], Object)),
     ?_assertEqual(#{<<"foo">> => #{<<"green">> => [#{<<"idx">> => 1},
                                                    #{<<"ham">> => 30, idx => 2}]}},
                   jsn:without([<<"foo.bar">>,
                                <<"foo.baz">>,
                                {<<"foo">>, <<"green">>, 1, <<"eggs">>}], Object)),
     ?_assertEqual(#{<<"foo">> => #{<<"bar">> => 99,
                                    <<"baz">> => #{<<"fum">> => <<"a">>},
                                    <<"green">> => [#{<<"idx">> => 1},
                                                    #{<<"ham">> => 30}]}},
                   jsn:without([<<"foo.baz.fun">>,
                                {<<"foo">>, <<"green">>, first, <<"eggs">>},
                                {<<"foo">>, <<"green">>, last, <<"idx">>}], Object)),
     ?_assertEqual(ProplistObject,
                   jsn:without([<<"nothing">>], ProplistObject)),
     ?_assertEqual([],
                   jsn:without([<<"foo">>, <<"nothing">>], ProplistObject)),
     ?_assertEqual([{<<"foo">>, [{<<"baz">>, [{<<"fun">>, <<"b">>},
                                              {<<"fum">>, <<"a">>}]}]}],
                   jsn:without([<<"foo.bar">>, [foo, green]], ProplistObject)),
     ?_assertEqual([{<<"foo">>, [{<<"green">>, [[{<<"idx">>, 1}],
                                                [{<<"ham">>, 30},{idx, 2}]]}]}],
                   jsn:without([<<"foo.bar">>,
                                <<"foo.baz">>,
                                {<<"foo">>, <<"green">>, 1, <<"eggs">>}], ProplistObject)),
     ?_assertEqual([{<<"foo">>, [{<<"bar">>, 99},
                    {<<"baz">>, [{<<"fum">>, <<"a">>}]},
                                 {<<"green">>, [[{<<"idx">>, 1}],
                                                [{<<"ham">>, 30}]]}]}],
                   jsn:without([<<"foo.baz.fun">>,
                                {<<"foo">>, <<"green">>, first, <<"eggs">>},
                                {<<"foo">>, <<"green">>, last, <<"idx">>}], ProplistObject)),
     ?_assertEqual(Eep18Object,
                   jsn:without([<<"nothing">>], Eep18Object)),
     ?_assertEqual({[]},
                   jsn:without([<<"foo">>, <<"nothing">>], Eep18Object)),
     ?_assertEqual({[{<<"foo">>, {[{<<"baz">>, {[{<<"fun">>, <<"b">>},
                                                 {<<"fum">>, <<"a">>}]}}]}}]},
                   jsn:without([<<"foo.bar">>, [foo, green]], Eep18Object)),
     ?_assertEqual({[{<<"foo">>, {[{<<"green">>, [{[{<<"idx">>, 1}]},
                                                  {[{<<"ham">>, 30},
                                                    {idx, 2}]}]}]}}]},
                   jsn:without([<<"foo.bar">>,
                                <<"foo.baz">>,
                                {<<"foo">>, <<"green">>, 1, <<"eggs">>}], Eep18Object)),
     ?_assertEqual({[{<<"foo">>, {[{<<"bar">>, 99},
                                   {<<"baz">>, {[{<<"fum">>, <<"a">>}]}},
                                   {<<"green">>, [{[{<<"idx">>, 1}]},
                                                  {[{<<"ham">>, 30}]}]}]}}]},
                   jsn:without([<<"foo.baz.fun">>,
                                {<<"foo">>, <<"green">>, first, <<"eggs">>},
                                {<<"foo">>, <<"green">>, last, <<"idx">>}], Eep18Object)),
     ?_assertEqual(StructObject,
                   jsn:without([<<"nothing">>], StructObject)),
     ?_assertEqual({struct, []},
                   jsn:without([<<"foo">>, <<"nothing">>], StructObject)),
     ?_assertEqual({struct,
                    [{<<"foo">>, {struct,
                                  [{<<"baz">>, {struct,
                                                [{<<"fun">>, <<"b">>},
                                                 {<<"fum">>, <<"a">>}]}}]}}]},
                   jsn:without([<<"foo.bar">>, [foo, green]], StructObject)),
     ?_assertEqual({struct,
                    [{<<"foo">>, {struct,
                                  [{<<"green">>, [{struct,[{<<"idx">>, 1}]},
                                                  {struct,[{<<"ham">>, 30},
     ...
     ?_assertEqual({struct,
                   ...
                   jsn:without([<<"foo.baz.fun">>,
                                {<<"foo">>, <<"green">>, first, <<"eggs">>},
                                {<<"foo">>, <<"green">>, last, <<"idx">>}], StructObject)),
     ?_assertEqual(#{array => []},
                   jsn:without([{<<"array">>, first}],
                               #{array => [1]})),
     ?_assertEqual(#{array => [1]},
                   jsn:without([{<<"array">>, first, <<"foo">>}],
                               #{array => [1]})),
     ?_assertEqual(#{array => []},
                   jsn:without([{<<"array">>, first, <<"foo">>}],
                               #{array => []})),
     ?_assertEqual(#{array => [1]},
                   jsn:without([{<<"array">>, last}],
                               #{array => [1,2]})),
     ?_assertEqual(#{<<"element">> => true},
                   jsn:without([{<<"element">>, last}],
                               #{<<"element">> => true})),
     ?_assertError(badarg, jsn:without([], {99})),
     ?_assertError(badarg, jsn:without({99}, #{}))].
